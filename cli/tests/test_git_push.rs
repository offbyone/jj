...
fn test_git_push_unambigous_remote() {
    ...
    let work_dir = test_env.work_dir("local");
    work_dir.run_jj(["new", "bookmark1"]).success();
    let add_new_git_remote = |remote: &str| {
        let remote_interpolated = format!("{remote}");
        test_env
            .run_jj_in(".", &["git", "init", &remote_interpolated])
            .success();
        let remote_dir = work_dir.dir(&remote_interpolated);
        let remote_git_dir = git_repo_dir_for_jj_repo(&remote_dir);
        work_dir
            .run_jj([
                "git",
                "remote",
    ...
    // Add some new remotes which could get matched on substrings.
    add_new_git_remote("upstreeam");
    add_new_git_remote("upstream");
    work_dir.run_jj(["new", "bookmark1"]).success();
    work_dir.run_jj(["desc", "-m", "Foo"]).success();
    work_dir
        .run_jj(["bookmark", "move", "bookmark1", "-r=@"])
        .success();
    let output = work_dir.run_jj(&["git", "push", "--remote", "upstream"]);
    insta::assert_snapshot!(output, @r"
        ");
}
