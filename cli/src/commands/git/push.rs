...
fn find_bookmarks_targeted_by_revisions<'a>(
    ...
    if use_default_revset {
        ...
        context.local_variables.insert(
            "remote",
            ExpressionNode {
                kind: ExpressionKind::StringPattern {
                    kind: "exact",
                    value: remote.as_str().to_string(),
                },
        ...
        let (mut expression, _) = revset::parse_with_modifier(&mut diags, &push_revset, &context)?;
        print_parse_diagnostics(ui, "Failed to expand 'remote' in expression", &diags)?;
        expression =
            expression.intersection(&RevsetExpression::bookmarks(StringPattern::everything()));
