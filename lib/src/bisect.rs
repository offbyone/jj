...
//! Bisect a range of commits of commits using the provided test function.
...
impl<'repo> Bisector<'repo> {
    ...
    /// Mark a commit as good, bad, or skipped, according to the test result in
    /// `result`.
    pub fn mark(&mut self, id: CommitId, test_result: TestResult) {
        match test_result {
            TestResult::Good => self.mark_good(id),
            TestResult::Bad => self.mark_bad(id),
            TestResult::Skip => self.mark_skipped(id),
        }
    }
    ...
    /// Find the next commit to test, or determine that there are no more steps.
    pub fn next_step(&mut self) -> Result<NextStep, BisectionError> {
        ...
        let to_test_expr = self
            ...
            .bisect()
            .latest(1);
        let to_test_set = to_test_expr.evaluate(self.repo)?;
        if let Some(commit) = to_test_set
            .iter()
            .commits(self.repo.store())
            .next()
            .transpose()?
        {
            Ok(NextStep::Test(commit))
        } else {
